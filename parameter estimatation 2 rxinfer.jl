...
# ╔═╡ 8ba32f2b-185b-4314-93af-fec99944f142
begin
	import Distributions
	using Plots, Random, PlutoUI
end

# ╔═╡ 75b03303-9252-4ada-8307-c98e32c1ce58
using Turing

# ╔═╡ 64fd0296-20b3-46f2-9e98-9b3b24bfbd30
md"""
# Generating data
"""

# ╔═╡ a7df22c7-0dfd-4003-b7de-c317f2bdd083
seed() = rand(Int)

# ╔═╡ 39f0fcda-929f-4451-80c6-7b04099a0dee
@bind secret_α Slider(-20.0:0.1:20.0; default=2.0)

# ╔═╡ 068c399d-0178-47e9-a85b-a15f39f7f4e8
@bind secret_β Slider(0.01:0.1:20.0; default=3.0)

# ╔═╡ d0b83863-b911-43ba-83ab-58d7ea4755e8
secret_distribution = Distributions.Cauchy(secret_α, secret_β)

# ╔═╡ 0db5cc7f-43c1-4626-8a18-8b3c3426df75
@bind N Slider(0:100; default=5)

# ╔═╡ 2ed49148-d9a9-42ab-b0e1-20cbc8d13d28
N

# ╔═╡ cb940a1c-ca88-43b8-952c-231cff27b57b
data = rand(MersenneTwister(seed()), secret_distribution, N)

# ╔═╡ 77cb2b17-43ac-4c4f-93bd-a031c61cb360
...
# ╔═╡ f9d2baa6-a790-494c-a747-e1a43a9164ee
lighthouse_plot()

# ╔═╡ 6a85b016-53b3-4805-b447-782e7df50a6b
@model function lighthouse_model(;observations, alpha_prior, beta_prior)

	alpha ~ alpha_prior
	beta ~ beta_prior

	for i in eachindex(observations)
		observations[i] ~ Cauchy(alpha, beta)
	end
end

# ╔═╡ e91bff21-a90f-46c6-9359-891cb11d5153
model = lighthouse_model(
		# alpha_prior = Normal(0.0, 20.0), 
		# beta_prior = Normal(10.0, 10.0),
		# alpha_prior = Uninformative(), 
		# beta_prior = Uninformative(),
		alpha_prior = Uniform(-20.0, 20.0), 
		beta_prior = Uniform(0.0, 25.0),
		# alpha_prior = Uninformative(), 
		# beta_prior = Uninformative(),
		# alpha_prior = Uninformative(), 
		# beta_prior = Uninformative(),
...
# ╔═╡ a45629ab-5517-451d-9cd7-3cb4dd7089b0
chain = sample(model, NUTS(), 20000,)

# ╔═╡ 6871a722-5ea6-4dd4-b07b-aa323801481a
histogram(chain)


# ╔═╡ faae9db9-9bb2-4d25-a0aa-29d9eccff21e
import Statistics

# ╔═╡ 56e6bf54-5b70-4840-bd5e-c181688cc189
Statistics.cov(hcat(chain[:alpha], chain[:beta]))

# ╔═╡ ac667df0-638b-4745-ae47-328ec7307a27
result = infer(
	model = lighthouse_model(
		# alpha_prior = Normal(0.0, 20.0), 
		# beta_prior = Normal(10.0, 10.0),
...
manifest_format = "2.0"
project_hash = "b6ab7f98405ad44a28609d0ea4d61e485ba1da24"
...
git-tree-sha1 = "d23ac9ecdf0ba3dfdd8ea22f42793fabc763afdc"
...
git-tree-sha1 = "2ebd971842f8ada0c6c9bd205ff0997961538110"
...
git-tree-sha1 = "40fd43cc5dd3fe76cf67f033b69e073207a91b7e"
...
git-tree-sha1 = "8b1247493543b972909755123859a7bae67828c4"
